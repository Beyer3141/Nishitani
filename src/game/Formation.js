...
export const FORMATIONS = {
    line(count, gameWidth, spacing = 70) {
        ...
        for (let i = 0; i < count; i++) {
            spawns.push({ x: Math.max(10, Math.min(gameWidth - 70, startX + i * spacing)), y: -60, delay: i * 150 });
        }
        return spawns;
    },
    ...
    circle(count, gameWidth, radius = 100) {
        ...
        for (let i = 0; i < count; i++) {
            ...
            spawns.push({ x: cx + Math.cos(angle) * radius - 30, y: -60, delay: i * 100 });
        }
        return spawns;
    },
    ...
    diamond(count, gameWidth, size = 80) {
        ...
        for (let r = 0; r < rows && idx < count; r++) {
            ...
            for (let c = 0; c < rowCount && idx < count; c++) {
                spawns.push({ x: Math.max(10, Math.min(gameWidth - 70, startX + c * size)), y: -60, delay: idx * 120 });
                idx++;
            }
        }
        return spawns;
    ...
    grid(rows, cols, gameWidth, spacingX = 70, spacingY = 60) {
        ...
        for (let r = 0; r < rows; r++) {
            for (let c = 0; c < cols; c++) {
                spawns.push({ x: Math.max(10, Math.min(gameWidth - 70, startX + c * spacingX)), y: -60 - r * spacingY, delay: (r * cols + c) * 100 });
            }
        }
        return spawns;
    },
    ...
    wave(count, gameWidth, amplitude = 50) {
        const spawns = [];
        for (let i = 0; i < count; i++) {
            spawns.push({ x: Math.max(10, Math.min(gameWidth - 70, gameWidth / 2 + Math.sin(i * 0.8) * amplitude - 30)), y: -60, delay: i * 300 });
        }
        return spawns;
    },
    ...
    random(count, gameWidth) {
        const spawns = [];
        for (let i = 0; i < count; i++) {
            spawns.push({ x: 10 + Math.random() * (gameWidth - 80), y: -60, delay: i * 400 + Math.random() * 200 });
        }
        return spawns;
    },
    ...
    sides(count, gameWidth) {
        const spawns = [];
        for (let i = 0; i < count; i++) {
            spawns.push({ x: i % 2 === 0 ? 10 : gameWidth - 70, y: -60, delay: i * 250 });
        }
        return spawns;
    },
